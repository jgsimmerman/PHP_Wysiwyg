...
class AdminLinks extends Controller {
    ...
    public function get() {
        ...
        while($row = $result->fetch_object()):
            ...
            $row->email = '<a href="' . url('admin/user-update/' . $row->user_id) . '"> ' . $row->email . '</a>';

            /* Location URL */
            if(!empty($row->location_url)) {
                $location_url = '<img src="https://www.google.com/s2/favicons?domain=' . parse_url($row->location_url)['host'] . '" class="img-fluid mr-1" />';
            ...
            $row->actions = get_admin_options_button('link', $row->link_id);
